...
class SA:
    ...
    def __init__(self, data, infinity=1<<17, initial_t=0, rate=0.999, stopping_t=0.0001, initial_fitness=1, initial_solution=None, iteration_bound=[1 << 17, 1 << 24], regularization_bound=(0.3, 3), learning_plot=False, silent_mode=False):
        ...
        # creatingg adjacency matrix from the array list initially given
        self.n = data.pop(0)
        ...
        # using initial solution provided or generating a new one
        if initial_solution:
    ...
    # for generating the initial solution (djikstra starting from node 0)
    ...
    def accept(self, candidate, random_acceptance=True):
        ...
        if candidate_cost < self.current_cost:
            ...
            res = True                                 # might need to check out if this line should be inside or outside the if block
